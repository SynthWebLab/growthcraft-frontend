...
export default function StudentBatchesPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 p-4 sm:p-0">
      ...
      {batches.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 px-4 text-center border border-dashed border-border rounded-xl bg-card">
          <Layers className="h-12 w-12 text-muted-foreground/30 mb-3" />
          <h3 className="font-semibold text-base sm:text-lg text-foreground">No batches assigned yet</h3>
        ...
        <div className="grid gap-4 sm:gap-6 grid-cols-1 md:grid-cols-2">
          {batches.map((batch) => (
            <DataCard key={batch.id} className="border-border/60 hover:shadow-md transition-shadow flex flex-col justify-between p-4 sm:p-6">
              <div>
                <div className="flex flex-col xs:flex-row xs:items-start justify-between gap-3 mb-4">
                  <div className="min-w-0">
                    <span className="inline-block font-mono text-[10px] sm:text-xs font-bold text-magenta bg-magenta/10 px-2 py-0.5 rounded w-fit">
                      {batch.code}
                    </span>
                    <h3 className="font-bold text-sm sm:text-base text-foreground mt-2 leading-snug truncate">
                      {batch.title}
                    </h3>
                    <p className="text-[10px] sm:text-xs text-muted-foreground uppercase mt-0.5 tracking-wide">
              ...
              <div className="pt-2">
                <p className="text-[10px] sm:text-xs font-semibold text-muted-foreground mb-2">Assigned Mentor</p>
                <div className="flex items-center justify-between p-2.5 sm:p-3 rounded-lg bg-marble/60 border border-border/50 gap-2">
                  ...
                  {batch.mentorEmail && (
                    <a
                      ...
                      className="h-8 w-8 rounded-full border border-border bg-white hover:text-magenta hover:border-magenta flex items-center justify-center text-muted-foreground transition-all shrink-0"
                      title="Email Mentor"
                    >
