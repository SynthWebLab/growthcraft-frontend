...
import { useStudentBootcampWorkspace, useSubmitBootcampProject } from "@/hooks/queries/useStudent";
import { PaymentCheckoutModal } from "@/components/dashboard/payment-checkout-modal";

export default function StudentBootcampWorkspacePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = use(params);

  // Fetch dynamic bootcamp workspace data from API
  const { data: workspaceRes, isLoading, isError } = useStudentBootcampWorkspace(slug);
  const submitMutation = useSubmitBootcampProject(slug);
  ...
  const DEFAULT_BOOTCAMP_PHASES = [
    { phase: 1, name: "Cohort Onboarding & Fundamentals", status: "Completed", description: "Orientation address, environment setup, and foundational architecture." },
    { phase: 2, name: "Core Stack Modules & Guided Labs", status: "Completed", description: "Deep-dive technical modules, database modeling, and live instructor labs." },
    { phase: 3, name: "Capstone Project Build & Code Audits", status: "Completed", description: "Build your industry capstone project with daily mentor reviews." },
    { phase: 4, name: "Final Demo, Evaluation & Certificate Sign-off", status: "Completed", description: "Capstones evaluation by jury, career guidance, and certificate distribution." },
  ];

  const phases = (workspaceData?.phases && workspaceData.phases.length > 0) ? workspaceData.phases : DEFAULT_BOOTCAMP_PHASES;
  ...
  const handleSelfCheckin = () => {
    ...
    toast.success("Check-in confirmed!", {
      description: "Your bootcamp attendance has been recorded."
    });
  };
      ...
      <div className="max-w-4xl mx-auto p-6 md:p-12 text-center space-y-6 my-12">
        ...
        <Card className="p-8 rounded-3xl border-2 border-amber-500/30 bg-gradient-to-b from-amber-500/5 via-white to-amber-500/5 shadow-xl space-y-6">
          ...
          <div className="space-y-2 max-w-md mx-auto">
            ...
            <h2 className="text-2xl font-extrabold text-foreground">Payment Required to Access Bootcamp Workspace</h2>
            <p className="text-sm text-muted-foreground leading-relaxed">
              You have registered for <strong className="text-foreground">{pageTitle}</strong>, but your enrollment payment is currently pending. Please complete the registration payment to unlock access to assigned mentors, campus check-in passes, and capstone project submission.
            </p>
          </div>
  ...
  const rawEnd = event?.endDate || "2026-06-30T18:00:00Z";
  ...
  const endDate = !isNaN(endObj.getTime()) ? endObj.toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "June 30, 2026 • 06:00 PM";
  ...
  const venue = isOnline 
    ...
    : (event?.venue || "GrowthCraft Campus Hub • Main Auditorium");
  
  const checkinCode = enrollment?.checkinCode || "GC-BTC-2026-9012";
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-7xl mx-auto p-4 sm:p-6 pb-16">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-3 max-w-3xl">
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
              ...
              {computedStatus === "Live" && (
                <span className="inline-flex items-center gap-1.5 text-[9px] sm:text-xs font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-3 py-1 rounded-full animate-pulse">
                  <span className="h-2 w-2 rounded-full bg-emerald-400" />
                  Live
                </span>
              )}
              {computedStatus === "Open" && (
                <span className="inline-flex items-center gap-1.5 text-[9px] sm:text-xs font-semibold text-blue-400 bg-blue-500/10 border border-blue-500/20 px-3 py-1 rounded-full">
                  <span className="h-2 w-2 rounded-full bg-blue-400" />
                  Open
                </span>
              )}
              {computedStatus === "Closed" && (
                <span className="inline-flex items-center gap-1.5 text-[9px] sm:text-xs font-semibold text-rose-400 bg-rose-500/10 border border-rose-500/20 px-3 py-1 rounded-full">
            ...
            <p className="text-slate-300 text-xs sm:text-sm md:text-base leading-relaxed">
              Welcome to your bootcamp workspace. Access live module sessions, collaborate with Admin-assigned mentors, track cohort milestones, and submit your capstone project.
            </p>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2.5 text-xs text-slate-300 pt-1.5">
              <div className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-xl border border-white/10 w-fit">
                ...
                <span className="truncate max-w-[280px] sm:max-w-none">Starts: {startDate}</span>
              </div>
              <div className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-xl border border-white/10 w-fit">
                <Clock className="h-4 w-4 text-magenta shrink-0" />
                <span className="truncate max-w-[280px] sm:max-w-none">Ends: {endDate}</span>
              </div>
              <div className="flex items-center gap-1.5 bg-white/5 px-3 py-1.5 rounded-xl border border-white/10 w-fit">
          ...
          <div className="flex flex-col sm:flex-row lg:flex-col gap-3 min-w-[200px] w-full lg:w-auto">
            ...
            <a href="#capstone-form" className="w-full">
            ...
            <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4 w-full">
              ...
              <div className="space-y-1.5 w-full min-w-0">
                <h3 className="font-bold text-sm sm:text-lg text-foreground flex flex-wrap items-center gap-2 leading-none">
                  {isOnline ? "Virtual Bootcamp Pass & Meeting" : "Campus Check-in Token"}
                ...
                <p className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed">
                  {isOnline 
                    ? "Click to join the virtual bootcamp live classroom or perform 1-click online check-in." 
                    : "Show this pass code to your GrowthCraft Bootcamp Mentor or click self check-in."}
                </p>
                
                {isOnline ? (
                  <div className="flex flex-col sm:flex-row sm:items-center gap-2.5 pt-1.5 w-full">
                    <a 
                      href={mentors[0]?.meetingLink || "https://meet.google.com/gc-bootcamp-room"} 
                      target="_blank" 
                      rel="noreferrer"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 sm:gap-6">
        ...
        {/* Left 2 Columns: Attendance Status, Timeline & Capstone Form */}
        <div className="lg:col-span-2 space-y-5 sm:space-y-6">
          ...
          <Card className="p-4 sm:p-6 rounded-2xl border border-border bg-white shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base sm:text-lg font-bold text-foreground">Attendance & Participation Status</h2>
              </div>
              <Badge variant="secondary" className={`w-fit font-bold px-2.5 py-0.5 text-[10px] sm:text-xs ${attendanceTagClass}`}>
                {attendanceTagLabel}
              </Badge>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
              <div className="p-3 sm:p-4 rounded-xl bg-slate-50 border border-slate-100/80">
                <span className="text-[10px] sm:text-xs text-muted-foreground block font-medium">Check-in Mode</span>
                <span className="text-xs sm:text-sm font-bold text-emerald-600 flex items-center gap-1 mt-0.5 truncate">
                  ...
                  {isOnline ? "Virtual Room" : "Campus QR"}
                </span>
              </div>
              <div className="p-3 sm:p-4 rounded-xl bg-slate-50 border border-slate-100/80">
                <span className="text-[10px] sm:text-xs text-muted-foreground block font-medium">Assigned Mentor</span>
                <span className="text-xs sm:text-sm font-bold text-foreground mt-0.5 block truncate">
                  {mentors[0]?.name || "Prof. Rajesh Sharma"}
                </span>
              </div>
              <div className="p-3 sm:p-4 rounded-xl bg-slate-50 border border-slate-100/80">
                <span className="text-[10px] sm:text-xs text-muted-foreground block font-medium">Bootcamp Track</span>
                <span className="text-xs sm:text-sm font-bold text-foreground mt-0.5 block truncate">Software Engineering</span>
              </div>
            </div>

            {!hasAttended && computedStatus !== "Closed" && (
              <div className="pt-1.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-emerald-500/5 p-3 rounded-xl border border-emerald-500/20">
                <span className="text-[11px] sm:text-xs font-medium text-emerald-800 flex items-center gap-1.5">
                  <AlertCircle className="h-4 w-4 text-emerald-600 shrink-0" />
                  Have you arrived at campus / joined the live room?
                </span>
                <Button size="sm" onClick={handleSelfCheckin} className="w-full sm:w-auto bg-emerald-600 text-white hover:bg-emerald-700 h-8 text-[11px] sm:text-xs px-3">
                  Check-in Now
                </Button>
              </div>
            )}
          ...
          {/* Bootcamp Milestones & Phase Schedule */}
          <Card className="p-4 sm:p-6 rounded-2xl border border-border bg-white shadow-sm space-y-4">
          ...
          <Card id="capstone-form" className="p-4 sm:p-6 rounded-2xl border border-border bg-white shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border pb-4">
              <div className="flex items-center gap-2">
                <Code2 className="h-5 w-5 text-magenta shrink-0" />
                <div>
                  <h2 className="text-base sm:text-lg font-bold text-foreground">Capstone Project Submission</h2>
                  <p className="text-[11px] sm:text-xs text-muted-foreground">Keep your team capstone repository and demo URL updated for mentor evaluation.</p>
                </div>
              </div>
              {isSubmitted && (
            ...
            <form onSubmit={handleSaveSubmission} className="space-y-4 pt-1">
              <div>
                <label className="text-xs font-semibold text-foreground block mb-1.5">Capstone Project Title</label>
                <input 
                  type="text" 
                  value={projectTitle}
                  onChange={(e) => setProjectTitle(e.target.value)}
                  placeholder="e.g. Enterprise Microservices Platform" 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    placeholder="https://github.com/org/bootcamp-capstone" 
                    className="w-full px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-xl border border-border bg-slate-50 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-magenta/30"
                    required
                  />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    placeholder="https://bootcamp-demo.vercel.app" 
                    className="w-full px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-xl border border-border bg-slate-50 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-magenta/30"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <input 
                  ...
                  placeholder="e.g. Next.js, Node.js, Docker, PostgreSQL" 
                  className="w-full px-3 py-2 sm:px-3.5 sm:py-2.5 rounded-xl border border-border bg-slate-50 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-magenta/30"
                />
              </div>

              <div>
                <label className="text-xs font-semibold text-foreground block mb-1.5">Capstone Overview / Mentor Notes</label>
                <textarea 
                  rows={3}
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="Briefly describe what your capstone project accomplishes..." 
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2">
                <p className="text-[11px] sm:text-xs text-muted-foreground flex items-center gap-1">
                  ...
                  Capstone submission is required for bootcamp certificate approval.
                </p>
                <Button 
                  type="submit" 
          ...
          <Card className="p-4 sm:p-6 rounded-2xl border border-border bg-white shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base sm:text-lg font-bold text-foreground">Assigned Mentors</h2>
              </div>
                ...
                <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100/80 gap-3">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] sm:text-xs text-muted-foreground truncate">{m.designation || m.areaOfExpertise || "Bootcamp Mentor"}</p>
                    </div>
                  </div>
                  {isOnline || m.meetingLink ? (
                    <a 
                      href={m.meetingLink || "https://meet.google.com/gc-bootcamp-room"} 
                      target="_blank" 
                      rel="noreferrer"
                      className="shrink-0"
                    >
          ...
          <Card className="p-4 sm:p-6 rounded-2xl border border-border bg-gradient-to-br from-amber-500/5 via-slate-50 to-magenta/5 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-base sm:text-lg font-bold text-foreground">Bootcamp Certificate</h2>
              </div>
            ...
            {!canUnlockCert ? (
              <div className="space-y-3">
                <p className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed">
                  Certificate download will unlock once you have <strong className="text-foreground">attended the bootcamp</strong> and <strong className="text-foreground">submitted your capstone project</strong>.
                </p>
                <div className="space-y-1.5 text-[11px] sm:text-xs text-slate-600 bg-white p-3 rounded-xl border border-border">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>Capstone Project: <strong>{isSubmitted ? "Submitted" : "Not Submitted"}</strong></span>
                  </div>
                </div>
                <Button 
                  disabled
              ...
              <div className="space-y-3">
                <p className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed">
                  Your attendance and capstone project submission have been logged. GrowthCraft Admins are verifying your details before issuing your official bootcamp certificate.
                </p>
                ...
                <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-900 space-y-1.5">
                  ...
                  <p className="text-[10px] text-amber-800 leading-relaxed">
                    Admin verification ensures valid capstone code repository and attendance proof.
                  </p>
                </div>
              ...
              <div className="space-y-3">
                <p className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed">
                  Congratulations! Your attendance and capstone project have been verified and approved by GrowthCraft Admin. Download your bootcamp certificate below.
                </p>
                ...
                <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-emerald-900 flex items-center gap-2">
                  ...
                  <span className="text-[11px] sm:text-xs font-medium">Verified by Admin • Serial No: GC-BTC-2026-9012</span>
                </div>

                <Button 
                  onClick={() => {
                    toast.success("Certificate download started!", {
                      description: "Your verified PDF bootcamp certificate is downloading."
                    });
                  }}
