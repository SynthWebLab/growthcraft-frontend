        ...
        {mentorsMock.map((mentor) => (
          <DataCard key={mentor._id} className="text-center h-full">
            <img
              src={mentor.photo}
              alt={mentor.name}
              className="w-14 h-14 sm:w-16 sm:h-16 rounded-full mx-auto mb-3 sm:mb-4 bg-muted"
            />
            ...
            <div className="flex flex-wrap items-center justify-center gap-1.5 mb-3 sm:mb-4">
              {mentor.expertiseTags.map((tag) => (
                <span
                  key={tag}
                  className="px-2 py-0.5 rounded-full bg-secondary/10 text-xs font-medium text-foreground"
                >
                  {tag}
                </span>
              ))}
